...
find_difference <- function(df, group, variable){
    ...
    if (difference == 0){
        print(paste("There is difference for variable", variable, "in group", group))
    } else{
        difference_percent <- round(difference/nrow(home_imprecise),2)
        print(paste0("There are ", difference_percent, "% difference for variable ", variable, " in group ", group))
