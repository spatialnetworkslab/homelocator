...
test_sample <- test_sample %>% mutate(GEOID = as.character(GEOID))
...
And function `prop_factor_nested()` allows you to calculate the proportion of categories for a variable inside the list-column and convert each categories to a new variable adding to the dataframe. 
...
colnmaes_to_nest <- colnmaes_data[-which(colnmaes_data == "GEOID")] 
...
df_scored <- df_expanded %>% 
  score_nested(., user = "u_id", location = "GEOID", keep_ori_vars = F,
...
df_score_summed <- df_scored %>% 
  score_summary(., user = "u_id", location = "GEOID", starts_with("s_"))
...
df_home <- df_score_summed %>% 
  extract_location(., user = "u_id", location = "GEOID", show_n_loc = 1, keep_score = F, score)

df_home %>% head(3)

# extract homes for users and keep scores of locations
df_home <- df_score_summed %>% 
  extract_location(., user = "u_id", location = "GEOID", show_n_loc = 1, keep_score = T, score)
...
identify_location(test_sample, user = "u_id", timestamp = "created_at", location = "GEOID", 
