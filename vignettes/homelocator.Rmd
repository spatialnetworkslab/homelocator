...
<p align="justify">The `homelocator` library is designed for identifying meaningful locations of user based on the spatial and temporal features contained within the mobility data, such as social media data, mobile phone data and so on.</p>

<p align="justify">The main objective for developing this package is to provide a consistent framework and interface for the adoption of different approaches so that researchers are able to write structured, algorithmic 'recipes' to identify meaningful locations according to their research requirements and so they are able to achieve an apples-to-apples comparison across approaches.</p>
...
The `nest_verbose()` and `unnest_verbose()` functions in `nest.R` script work in the same way as  [nest](https://www.rdocumentation.org/packages/tidyr/versions/0.8.3/topics/nest) and [unnest](https://www.rdocumentation.org/packages/tidyr/versions/0.8.3/topics/unnest) functions in [tidyverse](https://github.com/tidyverse/tidyverse) but with some additional status information such as the elapsed running time. 
...
The `enrich_timestamp()` in `enrich_timestamp.R` script creates additional variables that are derived from the timestamp column. These include the year, month, day, day of the week and hour of the day. These are often used/needed as intermediate variables in home location algorithms. 
  ...
  - `tz`: a character string containing the time zone to convert to. This should be one of the time zones [recognized by R](https://stat.ethz.ch/R-manual/R-devel/library/base/html/timezones.html). The default `tz` is set to `Asia/Singapore`.
...
The `summarise_nested()` function in the `summarise.R` script works similar to dplyr's regular `summarise` function but operates within a nested tibble.  `summarise_double_nested()` conversely operates within a double-nested tibble.
...
### Mutate 
The `mutate_verbose()` function in `filter.R` script works in the same way as [mutate](https://www.rdocumentation.org/packages/dplyr/versions/0.5.0/topics/mutate) function in [tidyverse](https://github.com/tidyverse/tidyverse) but with additional information about the elapsed running time. 
...
The function `prop_factor_nested()` allows you to calculate the proportion of categories for a variable inside the list-column and convert each categories to a new variable adding to the dataframe. For example, [ADD EXAMPLE TO EXPLAIN USE]
...
### Score 
The `score_nested()` function in `score.R` script allows you to give a weighted value for one or more variables in a nested dataframe. 
...
```
==
[NOTE FROM ATE: STILL NEED TO READ BELOW THIS LINE]
==
### Extract locations 
