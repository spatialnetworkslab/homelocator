...
df_removed_active_users <- remove_top_users(df_summarize_nested,
  user = "u_id",
...
df_filtered <- filter_verbose(df_removed_active_users,
  user = "u_id",
...
df_filter_nested <- filter_nested(df_filtered,
  user = "u_id",
...
df_expanded <- df_cleaned %>%
  mutate_nested(
    wd_or_wk = if_else(wday %in% c(1, 7), "weekend", "weekday"),
...
df_scored <- df_expanded %>%
  score_nested(.,
    user = "u_id", location = "grid_id", keep_original_vars = F,
...
identify_location(test_sample,
  user = "u_id", timestamp = "created_at",
