...
hm_full <- bind_rows(hm_hlc, hm_osn, hm_mpd, hm_frq) %>% 
...
tm_shape(df_subzone)+
  ...
  tm_layout(
   ...
   tm_facets(by = "method", ncol = 2, nrow = 2)
...
<!-- df_same <- hm_full %>% filter(u_id %in% same_users) %>%  -->
...
# dens_home_wk <- user_1hm_tweets %>% 
#   transmute(wday = wday(created_at)) %>% 
#   mutate(wk = if_else(wday %in% c(1, 7), 1, 0))
```
...
# #ggmap 
# Sg_lon_lat <- geocode(c("Singapore"))
# sg_map <- get_map(location = Sg_lon_lat, zoom = 11, color = "bw")
# ggmap(sg_map, extent = "normal", maprange=FALSE) %+% test + aes(x = lon, y = lat)
# 
# p_tweets_density <- ggplot(zone_4326) + 
#   geom_sf() + 
#   geom_density2d(data = tweets_with_coords, aes(x = lon, y = lat)) +
#    stat_density2d(data = tweets_with_coords, aes(x = lon, y = lat, fill = ..level.., alpha = ..level..),
#                   size = 0.01, bins = 16, geom = 'polygon') +
#    scale_fill_gradient(low = "green", high = "red") +
#    scale_alpha(range = c(0.00, 0.5), guide = FALSE) +
#   labs(x = "Lon", y = "Lat", 
#        title = "Tweets density in Singapore from 2012 to 2015") + 
#   theme(
#     legend.position = "bottom",
#     panel.border = element_blank(),  
#     panel.grid.major = element_blank(),
...
#   )
# 
# tweets_with_coords <- readRDS(file = here::here("data/tweets_with_coords.rds"))
# users_with_coords <- readRDS(file = here::here("data/users_with_coords.rds"))
# 
# grids_coords <- grids %>% 
#   st_transform(4326) %>% 
#   st_centroid() %>% 
...
#   setNames(c("lon", "lat")) 
# 
# grids_2 <- grids %>% st_set_geometry(NULL) %>% bind_cols(grids_coords)
# 
# t <- tweets_with_coords %>% 
#   select(c(u_id, grid_id)) %>% 
#   unique() %>% left_join(grids_2)
...
tm_shape(df_subzone) + 
  tm_borders(col = "grey20") + 
  tm_shape(grid_u) + 
  tm_fill("n_users", palette = color8,
          n = 8, style = "fisher", 
          legend.hist = TRUE,
          title = "Number of Residents"
          ) +
  tm_layout(
            title = "Singapore Residents by Planing Area in 2015",
            title.position = c("left", "top"),
    title.size = 0.8,
            legend.outside = F,
            legend.position = c("right", "bottom"),
...
tm_shape(df_subzone) + 
  tm_borders(col = "grey20") + 
  tm_shape(df_subzone) + 
  tm_text(text = "PLN_AREA_N", size = 0.3)
