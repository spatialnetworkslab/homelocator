...
df_sub <- df %>% 
  ...
  mutate(year = year(created_at), 
         ...
         time_frame = if_else(hour_of_day >= 2 & hour_of_day < 8, "Rest time", if_else(hour_of_day >= 8 & hour_of_day < 19, "Active time", "Leisure time"))) 
head(df_sub)
...
df_rest <- df_clean_3 %>% filter(time_frame == "Rest time")
df_active <- df_clean_3 %>% filter(time_frame == "Active time")
df_leisure <- df_clean_3 %>% filter(time_frame == "Leisure time")
...
user_rest <- df_rest$u_id  #users in rest time dataframe 
user_leisure <- df_leisure$u_id # users in leisure time dataframe
# user_leisure covers user_rest, which means there are others users in user_leisure that only have leisure time 
user_onlyhave_leisure <- setdiff(user_leisure, user_rest)


# get new subset of users that have both rest time and leisure time 
df_rest_leisure <- df_clean_3 %>% 
  filter(u_id %in% df_rest$u_id) %>% 
  filter(time_frame != "Active time")


test <- df_rest_leisure %>% 
  group_by(u_id) %>% 
  nest()
...
#   group_by(GEOID, unique_day) %>% 
#   summarise(counts = n()) %>% 
#   mutate(weighted_counts = weight_rest*counts) %>% 
#   group_by(GEOID) %>% 
#   summarise(weighted_counts_sum = sum(weighted_counts))
# 
# df_clean_3 %>% filter(u_id %in% setdiff(user_leisure, user_rest)) %>% pull(., time_frame) %>% unique()
# df_clean_3 %>% filter(u_id %in% user_rest) %>% pull(., time_frame) %>% unique()
```
