---
title: 'SG Anonymity Tweets: Home Location'
...
tweets <- readRDS(here::here("data/8_df_aggregated_hex.rds"))
...
#homelocator recipe
if (file.exists(here::here("data/anonymity_hm_hlc.csv"))){
  hm_hlc <- read_csv(here::here("data/anonymity_hm_hlc.csv")) %>% mutate(name = "HLC")
} else {
  hm_hlc <- homelocator::identify_home(tweets, user = "u_id", timestamp = "new_created_at", location = "grid_id", recipe = "homelocator")
  write_csv(hm_hlc, path = here::here("data/anonymity_hm_hlc.csv"))
}

#online social network recipe
if (file.exists(here::here("data/anonymity_hm_osn.csv"))) {
  hm_osn <- read_csv(here::here("data/anonymity_hm_osn.csv")) %>% mutate(name = "OSN")
} else{
  hm_osn <- homelocator::identify_home(tweets, user = "u_id", timestamp = "new_created_at", location = "grid_id", recipe = "OSN")
  write_csv(hm_osn, path = here::here("data/anonymity_hm_osn.csv"))
}

#mobile positioning data
if (file.exists(here::here("data/anonymity_hm_mpd.csv"))){
  hm_mpd <- read_csv(here::here("data/anonymity_hm_mpd.csv")) %>% mutate(name = "MPD")
} else {
  hm_mpd <- homelocator::identify_home(tweets, user = "u_id", timestamp = "new_created_at", location = "grid_id", recipe = "MPD")
  write_csv(hm_mpd, path = here::here("data/anonymity_hm_mpd.csv"))
}

#frequency
if (file.exists(here::here("data/anonymity_hm_smp.csv"))){
  hm_frq <- read_csv(here::here("data/anonymity_hm_smp.csv")) %>% mutate(name = "FREQ")
} else {
  hm_frq <- homelocator::identify_home(tweets, user = "u_id", timestamp = "new_created_at", location = "grid_id", recipe = "FREQ")
  write_csv(hm_frq, path = here::here("data/anonymity_result_smp.csv"))
}
...
# Tweets in different areas in SG
```{r}
sg_info <- tweets %>% 
  st_set_geometry(NULL) %>% 
  mutate(wday = wday(created_at, label = T, abbr = T),
         hr = hour(created_at))
sg_wday <- sg_info %>% 
  group_by(wday) %>% 
  dplyr::summarise(n_tweets_sg = n_distinct(id),
                   n_users_sg = n_distinct(u_id))
df_subzone_grids <- st_join(df_subzone, grids)
## central
central_grids_id <- df_subzone_grids %>% filter(REGION_N == "CENTRAL REGION") %>% pull(grid_id) %>% unique()
central_pln_areas <- df_subzone_grids %>% filter(REGION_N == "CENTRAL REGION") %>% pull(PLN_AREA_N) %>% unique()
tweets_in_central <- tweets %>% 
  filter(grid_id %in% central_grids_id) %>% 
  dplyr::select(id, u_id, created_at) %>% 
  mutate(wday = wday(created_at, label = T, abbr = T)) %>% 
  mutate(hr = hour(created_at)) %>% 
  mutate(timeframe = if_else(hr %in% seq(9, 18, 1), "Work Time", "Rest Time"))
central_wday <- tweets_in_central %>% 
  st_set_geometry(NULL) %>% 
  group_by(wday) %>% 
  dplyr::summarise(n_tweets = n_distinct(id),
                   n_users = n_distinct(u_id)) 

wday_central <- central_wday %>% 
  left_join(., sg_wday) %>% 
  mutate(Prop = (n_tweets/n_users)/(n_tweets_sg/n_users_sg)) %>% 
  dplyr::select(wday, Prop) %>% 
  spread(key = "wday", value = "Prop")
wday_central %>% 
  rbind(rep(0.6, 7), rep(0.4, 7), .) %>% 
    radarchart_plot(., color = "green", caxislabel_range = seq(0.4, 0.6, 0.05), "title")

## west 
west_grids_id <- df_subzone_grids %>% filter(REGION_N == "WEST REGION") %>% pull(grid_id) %>% unique()
west_pln_areas <- df_subzone_grids %>% filter(REGION_N == "WEST REGION") %>% pull(PLN_AREA_N) %>% unique()
tweets_in_west <- tweets %>% 
  st_set_geometry(NULL) %>% 
  filter(grid_id %in% west_grids_id) %>% 
  dplyr::select(id, u_id, created_at) %>% 
  mutate(wday = wday(created_at, label = T, abbr = T)) %>% 
  mutate(hr = hour(created_at)) %>% 
  mutate(timeframe = if_else(hr %in% seq(9, 18, 1), "Work Time", "Rest Time"))
west_wday <- tweets_in_west %>% 
  group_by(wday) %>% 
  dplyr::summarise(n_tweets = n_distinct(id),
                   n_users = n_distinct(u_id)) 
...
wday_west <- west_wday %>% 
  ...
  spread(key = "wday", value = "Prop")
wday_west %>% 
  rbind(rep(1, 7), rep(0.8, 7), .) %>% 
    radarchart_plot(., color = "blue", caxislabel_range = seq(0.8, 1, 0.05), "title")


## east 
east_grids_id <- df_subzone_grids %>% filter(REGION_N == "EAST REGION") %>% pull(grid_id) %>% unique()
east_pln_areas <- df_subzone_grids %>% filter(REGION_N == "EAST REGION") %>% pull(PLN_AREA_N) %>% unique()
tweets_in_east <- tweets %>% 
  st_set_geometry(NULL) %>% 
  filter(grid_id %in% east_grids_id) %>% 
  dplyr::select(id, u_id, created_at) %>% 
  mutate(wday = wday(created_at, label = T, abbr = T)) %>% 
  mutate(hr = hour(created_at)) %>% 
  mutate(timeframe = if_else(hr %in% seq(9, 18, 1), "Work Time", "Rest Time"))
east_wday <- tweets_in_east %>% 
  group_by(wday) %>% 
  dplyr::summarise(n_tweets = n_distinct(id),
                   n_users = n_distinct(u_id)) 
...
wday_east %>% 
  rbind(rep(0.6, 7), rep(0.4, 7), .) %>% 
    radarchart_plot(., color = "orange", caxislabel_range = seq(0.4, 0.6, 0.05), "title")



## NORTH-EAST REGION
north_east_grids_id <- df_subzone_grids %>% filter(REGION_N == "NORTH-EAST REGION") %>% pull(grid_id) %>% unique()
north_east_pln_areas <- df_subzone_grids %>% filter(REGION_N == "NORTH-EAST REGION") %>% pull(PLN_AREA_N) %>% unique()
tweets_in_north_east <- tweets %>% 
  st_set_geometry(NULL) %>% 
  filter(grid_id %in% east_grids_id) %>% 
  dplyr::select(id, u_id, created_at) %>% 
  mutate(wday = wday(created_at, label = T, abbr = T)) %>% 
  mutate(hr = hour(created_at)) %>% 
  mutate(timeframe = if_else(hr %in% seq(9, 18, 1), "Work Time", "Rest Time"))
north_east_wday <- tweets_in_north_east %>% 
  group_by(wday) %>% 
  dplyr::summarise(n_tweets = n_distinct(id),
                   n_users = n_distinct(u_id)) 

wday_north_east <- north_east_wday %>% 
  left_join(., sg_wday) %>% 
  mutate(Prop = (n_tweets/n_users)/(n_tweets_sg/n_users_sg)) %>% 
  dplyr::select(wday, Prop) %>% 
  spread(key = "wday", value = "Prop")
wday_north_east %>% 
  rbind(rep(0.6, 7), rep(0.4, 7), .) %>% 
    radarchart_plot(., color = "red", caxislabel_range = seq(0.4, 0.6, 0.05), "title")


## NORTH
north_grids_id <- df_subzone_grids %>% filter(REGION_N == "NORTH REGION") %>% pull(grid_id) %>% unique()
north_pln_areas <- df_subzone_grids %>% filter(REGION_N == "NORTH REGION") %>% pull(PLN_AREA_N) %>% unique()
tweets_in_north <- tweets %>% 
  st_set_geometry(NULL) %>% 
  filter(grid_id %in% east_grids_id) %>% 
  dplyr::select(id, u_id, created_at) %>% 
  mutate(wday = wday(created_at, label = T, abbr = T)) %>% 
  mutate(hr = hour(created_at)) %>% 
  mutate(timeframe = if_else(hr %in% seq(9, 18, 1), "Work Time", "Rest Time"))
north_wday <- tweets_in_north_east %>% 
  group_by(wday) %>% 
  dplyr::summarise(n_tweets = n_distinct(id),
                   n_users = n_distinct(u_id)) 

wday_north <- north_east_wday %>% 
  left_join(., sg_wday) %>% 
  mutate(Prop = (n_tweets/n_users)/(n_tweets_sg/n_users_sg)) %>% 
  dplyr::select(wday, Prop) %>% 
  spread(key = "wday", value = "Prop")
wday_north %>% 
  rbind(rep(0.6, 7), rep(0.4, 7), .) %>% 
    radarchart_plot(., color = "purple", caxislabel_range = seq(0.4, 0.6, 0.05), "title")

```

## tweets in sg in different hour 
```{r}
sg_hr <- sg_info %>% 
  group_by(hr) %>% 
  dplyr::summarise(n_tweets_sg = n_distinct(id),
...
## central
central_hr <- tweets_in_central %>% 
  st_set_geometry(NULL) %>% 
  group_by(hr) %>% 
  dplyr::summarise(n_tweets = n_distinct(id),
                   n_users = n_distinct(u_id)) 

hr_central <- central_hr %>% 
  left_join(., sg_hr) %>% 
  mutate(Prop = (n_tweets/n_users)/(n_tweets_sg/n_users_sg)) %>% 
  dplyr::select(hr, Prop) %>% 
  spread(key = "hr", value = "Prop")
hr_central %>% 
  rbind(rep(0.8, 24), rep(0.4, 24), .) %>% 
    radarchart_plot(., color = "green", caxislabel_range = seq(0.4, 0.8, 0.1), "title")

## west 
west_hr <- tweets_in_west %>% 
  group_by(hr) %>% 
  dplyr::summarise(n_tweets = n_distinct(id),
                   n_users = n_distinct(u_id)) 
...
hr_west <- west_hr %>% 
  ...
  spread(key = "hr", value = "Prop")
hr_west %>% 
  rbind(rep(1.2, 24), rep(0.8, 24), .) %>% 
    radarchart_plot(., color = "blue", caxislabel_range = seq(0.8, 1.2, 0.1), "title")


## east 
east_hr <- tweets_in_east %>% 
  group_by(hr) %>% 
  dplyr::summarise(n_tweets = n_distinct(id),
                   n_users = n_distinct(u_id)) 
...
hr_east <- east_hr %>% 
  ...
  spread(key = "hr", value = "Prop")
hr_east %>% 
  rbind(rep(0.5, 7), rep(0.8, 7), .) %>% 
    radarchart_plot(., color = "orange", caxislabel_range = seq(0.5, 0.8, 0.05), "title")



## NORTH-EAST REGION
north_east_hr <- tweets_in_north_east %>% 
  group_by(hr) %>% 
  dplyr::summarise(n_tweets = n_distinct(id),
                   n_users = n_distinct(u_id)) 

hr_north_east <- north_east_hr %>% 
  left_join(., sg_hr) %>% 
  mutate(Prop = (n_tweets/n_users)/(n_tweets_sg/n_users_sg)) %>% 
  dplyr::select(hr, Prop) %>% 
  spread(key = "hr", value = "Prop")
hr_north_east %>% 
  rbind(rep(0.8, 7), rep(0.5, 7), .) %>% 
    radarchart_plot(., color = "red", caxislabel_range = seq(0.5, 0.8, 0.05), "title")


## NORTH
north_hr <- tweets_in_north_east %>% 
  group_by(hr) %>% 
  dplyr::summarise(n_tweets = n_distinct(id),
                   n_users = n_distinct(u_id)) 

hr_north <- north_east_hr %>% 
  left_join(., sg_hr) %>% 
  mutate(Prop = (n_tweets/n_users)/(n_tweets_sg/n_users_sg)) %>% 
  dplyr::select(hr, Prop) %>% 
  spread(key = "hr", value = "Prop")
hr_north %>% 
  rbind(rep(0.8, 7), rep(0.5, 7), .) %>% 
    radarchart_plot(., color = "purple", caxislabel_range = seq(0.5, 0.8, 0.05), "title")
...
```{r}
tweets <- read_csv(here::here("data/sg_test.csv"))
tweets <- tweets %>% 
  nest(-u_id) %>% 
  sample_n(500) %>% 
  unnest()
...
homes <- read_csv(here::here("data/hm_hlc.csv")) %>% sample_n(12) %>% 
  mutate(Col = brewer.pal(n = 12, name = "Paired")) %>% 
  left_join(., grids, by = c("home" = "grid_id")) %>% 
  st_as_sf() 
homes <- homes %>% 
  st_centroid() %>% 
  st_transform(., "+init=epsg:4326") %>% 
    ...
    as.data.frame() %>% 
    setNames(c("long", "lat")) %>% 
    bind_cols(., homes %>% st_set_geometry(NULL))

poiIcons <- awesomeIconList(
  `#A6CEE3` = makeAwesomeIcon(icon = "home", markerColor = brewer.pal(n = 12, name = "Paired")[1], library = 'fa', iconColor = 'black'),
  `#1F78B4` = makeAwesomeIcon(icon = "home", markerColor = brewer.pal(n = 12, name = "Paired")[2], library = 'fa', iconColor = 'black'),
  `#B2DF8A` = makeAwesomeIcon(icon = "home", markerColor = brewer.pal(n = 12, name = "Paired")[3], library = 'fa', iconColor = 'black'),
  `#33A02C` = makeAwesomeIcon(icon = "home", markerColor = brewer.pal(n = 12, name = "Paired")[4], library = 'fa', iconColor = 'black'),
  `#FB9A99` = makeAwesomeIcon(icon = "home", markerColor = brewer.pal(n = 12, name = "Paired")[5], library = 'fa', iconColor = 'black'),
  `#E31A1C` = makeAwesomeIcon(icon = "home", markerColor = brewer.pal(n = 12, name = "Paired")[6], library = 'fa', iconColor = 'black'),
  `#FDBF6F` = makeAwesomeIcon(icon = "home", markerColor = brewer.pal(n = 12, name = "Paired")[7], library = 'fa', iconColor = 'black'),
  `#FF7F00` = makeAwesomeIcon(icon = "home", markerColor = brewer.pal(n = 12, name = "Paired")[8], library = 'fa', iconColor = 'black'),
  `#CAB2D6` = makeAwesomeIcon(icon = "home", markerColor = brewer.pal(n = 12, name = "Paired")[9], library = 'fa', iconColor = 'black'),
  `#6A3D9A` = makeAwesomeIcon(icon = "home", markerColor = brewer.pal(n = 12, name = "Paired")[10], library = 'fa', iconColor = 'black'),
  `#FFFF99` = makeAwesomeIcon(icon = "home", markerColor = brewer.pal(n = 12, name = "Paired")[11], library = 'fa', iconColor = 'black'),
  `#B15928` = makeAwesomeIcon(icon = "home", markerColor = brewer.pal(n = 12, name = "Paired")[12], library = 'fa', iconColor = 'black')
)
