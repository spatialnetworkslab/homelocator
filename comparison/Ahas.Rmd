...
#df_step3 has 10298 users 
#first iterated result   
oneHome <- anchor_results %>% filter(n_geoid == 1 & anchor_type == "home") # 1238 users 
oneWork <- anchor_results %>% filter(n_geoid == 1 & anchor_type == "work place") # 168 users 
oneHome_oneWork <- anchor_results %>% filter(n_geoid == 2 & anchor_type_n == 2) # 2058 users 

twoHome <- anchor_results %>% filter(n_geoid == 2) %>% filter(anchor_type_n == 1) %>% filter(anchor_type == "home") # 6634 users
twoWork <- anchor_results %>% filter(n_geoid == 2) %>% filter(anchor_type_n == 1) %>% filter(anchor_type == "work place") # 200 users
