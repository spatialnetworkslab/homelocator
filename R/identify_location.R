...
identify_location <- function(df, user = "u_id", timestamp = "created_at", location = "loc_id", recipe,
  ...
  ## recipe: HMLC
  if (recipe == "HMLC") {
    output <- recipe_HMLC(df_enriched,
      user = user, timestamp = timestamp, location = location,
  ...
  ## recipe: FREQ
  if (recipe == "FREQ") {
    output <- recipe_FREQ(df_enriched,
      user = user, timestamp = timestamp, location = location, show_n_loc,
  ...
  ## recipe: OSNA
  if (recipe == "OSNA") {
    output <- recipe_OSNA(df_enriched,
      user = user, timestamp = timestamp, location = location, show_n_loc = show_n_loc,
      keep_score = keep_score, use_default_threshold = use_default_threshold, rm_topNpct_user = rm_topNpct_user
    )
  }
  ...
  if (recipe == "APDM") {
    message(paste(
      emo::ji("exclamation"), "Please make sure you have loaded the neighbors table before you use APDM recipe.\nThe table should have one column named", location,
    ...
    output <- recipe_APDM(df_enriched, df_neighbors,
      user = user, timestamp = timestamp, location = location,
...
recipe_HMLC <- function(df, user = "u_id", timestamp = "created_at", location = "loc_id", show_n_loc,
  ...
  df_match_loc_condition <- df_match_user_condition %>%
    summarise_double_nested(.,
      nest_cols = colnmaes_to_nest, # summarise on location level
      n_points_loc = n(),
    ...
    filter_verbose(.,
      user = user,
      n_points_loc > threshold_n_points_loc & n_hours_loc > threshold_n_hours_loc & n_days_loc > threshold_n_days_loc & period_loc > threshold_period_loc
    ) %>% # each use at least has threshold_n_points_loc data points sent at threshold_n_hours_loc different hours for at least threshold_n_days_loc different days at each location, the whole time period is at least more than threshold_period_loc days
    summarise_nested(
      n_wdays_loc = n_distinct(wday), # add new variable after matching the conditions
  ...
  df_scored <- df_expanded %>%
    score_nested(.,
      user = user, location = location, # score each variables with weight
...
recipe_OSNA <- function(df, user = "u_id", timestamp = "created_at", location = "loc_id", show_n_loc,
  ...
  df_timeframe_nested <- df_match_user_condition %>%
    summarise_double_nested(.,
      nest_cols = colnmaes_to_nest,
      n_points_timeframe = n()
    ) %>% # number of data points at the timeframe
...
recipe_APDM <- function(df, df_neighbors, user = "u_id", timestamp = "created_at", location = "loc_id", keep_score = F, use_default_threshold = T) {
  ...
  df_with_start_point <- df_top5_locs %>%
    summarise_double_nested(.,
      nest_cols = colnmaes_to_nest,
      start_point = min({{ timestamp_expr }})
    ) # get the start data point of a day
  ...
  df_with_home_type <- df_with_start_point %>%
    ...
    summarise_nested(
      mean_start_time = mean(start_time),
  ...
  output <- extract_location(df_one_home %>% dplyr::select(-n_home),
    user = user,
  ...
  decide_home <- function(data) {
    ...
    home1 <- homes[1]
    home1_neighbors <- df_neighbors %>%
      filter({{ location_expr }} == home1) %>%
      pull(neighbor) %>%
      unlist() # get the neighboring locations of first home
    home2 <- homes[2]
